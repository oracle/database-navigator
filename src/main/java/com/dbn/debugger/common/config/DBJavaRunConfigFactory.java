...
public abstract class DBJavaRunConfigFactory<T extends DBJavaRunConfigType, C extends DBJavaRunConfig> extends DBRunConfigFactory<T, C> {
    ...
    public Icon getIcon(@NotNull RunConfiguration configuration) {
        ...
        JavaExecutionInput executionInput = runConfiguration.getExecutionInput();
        if (executionInput == null || runConfiguration.getCategory() != DBRunConfigCategory.CUSTOM) {
            ...
            DBJavaMethod method = methodRef.get();
            return method == null ? methodRef.getObjectType().getIcon() : method.getIcon();
        }
    }

    public abstract C createConfiguration(DBJavaMethod method);
}
