...
public enum PromptAction implements Presentable {
    ...
    public boolean is(String identifier) {
        return Strings.equalsIgnoreCase(apiId, identifier) ||
                Strings.equalsIgnoreCase(getId(), identifier);
    }

    public static PromptAction get(String identifier) {
        for (PromptAction action : values()) {
            if (action.is(identifier)) return action;
        }
