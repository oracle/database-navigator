...
public class SelectAiEditorNotificationProvider extends EditorNotificationProvider<SelectAiEditorNotificationPanel> {
    ...
    public SelectAiEditorNotificationProvider() {
    ...
    @NotNull
    @Override
    public Key<SelectAiEditorNotificationPanel> getKey() {
        return KEY;
    }
    ...
    public SelectAiEditorNotificationPanel createComponent(@NotNull VirtualFile file, @NotNull FileEditor fileEditor, @NotNull Project project) {
        ...
        AssistantState assistantState = assistantManager.getAssistantState(connection.getConnectionId(), AssistantType.SELECT_AI);
        if (!assistantState.isSupported()) return null;

        if (assistantState.getAcknowledgement() != FeatureAcknowledgement.NONE) return null;

        return new SelectAiEditorNotificationPanel(file, fileEditor, project);
    }
}
