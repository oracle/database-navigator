# DBN Portuguese Glossary

Canonical terminology decisions for Portuguese DBN translation work. Keep this file compact and promote only stable, reusable terms here.

| Source term | Portuguese term | Keep English | Context / notes |
| --- | --- | --- | --- |
| action | ação | No | UI actions and accessibility descriptions. |
| AI | IA | No | Standard Portuguese abbreviation. |
| API key | chave de API | Yes, partial | Common Portuguese technical UI term. |
...
| connectivity | conectividade | No | Connection testing and diagnostics. |
| compartment | compartimento | No | OCI compartment. |
| configuration | configuração | No | Settings/setup surfaces. |
| connection | conexão | No | Database connection setup and runtime connection. |
| constraint | restrição | No | Database constraints. |
...
| dataset | conjunto de dados | No | Assistant/profile datasets. |
| debug session | sessão de depuração | No | Debugger progress and log output. |
...
| endpoint | endpoint | Yes | OCI endpoint IDs. |
| event listener | listener de eventos | Yes, partial | Event notification registration. |
| execution plan | plano de execução | No | SQL explain-plan actions. |
| execution privilege | privilégio de execução | No | Database grants. |
...
| provider | provedor | No | AI provider. |
| pseudo column | pseudocoluna | No | JDBC metadata / database column setting. |
| reference | referência | No | Reference identifiers. |
| region | região | No | OCI region. |
...
| role | role | Yes | Database role in existing bundle. |
| runtime | runtime | Yes | OCI runtime type. |
| savepoint | ponto de salvamento | No | Transaction savepoint notifications. |
...
| staging table | tabela de staging | Yes, partial | Vector embedding staging table. |
| synonym | sinônimo | No | Database synonym object. |
| table | tabela | No | Database table. |
| tablespace | tablespace | Yes | Oracle tablespace quota. |
| tenancy | tenancy | Yes | OCI tenancy term in existing bundle. |
| timestamp | carimbo de data/hora | No | Event notification columns. |
| TNS / tnsnames.ora | TNS / tnsnames.ora | Yes | Oracle network configuration terms. |
| token | token | Yes | Auth token. |
| tool | ferramenta | No | Assistant/MCP tools. |
| transaction commit | confirmação | No | Database transaction command. |
