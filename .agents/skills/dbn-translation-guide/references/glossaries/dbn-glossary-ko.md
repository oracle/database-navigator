...
Canonical terminology decisions for Korean DBN translation work. Keep this file compact and promote only stable, reusable terms here.

| Source term | Korean term | Keep English | Context / notes |
| --- | --- | --- | --- |
| action | 작업 | No | UI actions and accessibility descriptions. |
| AI | AI | Yes | Standard technical abbreviation. |
| API key | API 키 | Yes, partial | Common Korean technical UI term. |
| argument | 인수 | No | Command/function arguments. |
...
| connectivity | 연결성 | No | Connection testing and diagnostics. |
| compartment | 구획 | No | OCI compartment. |
| configuration | 구성 | No | Settings/setup surfaces. |
| connection | 연결 | No | Database connection setup and runtime connection. |
| constraint | 제약 조건 | No | Database constraints. |
| credential | 자격 증명 | No | Authentication credential objects. |
...
| dataset | 데이터셋 | No | Assistant/profile datasets. |
| debug session | 디버그 세션 | No | Debugger progress and log output. |
...
| endpoint | 엔드포인트 | No | OCI endpoint IDs. |
| event listener | 이벤트 리스너 | No | Event notification registration. |
| execution plan | 실행 계획 | No | SQL explain-plan actions. |
| execution privilege | 실행 권한 | No | Database grants. |
...
| gutter | 거터 | No | IntelliJ editor gutter. |
| identifier | 식별자 | No | SQL/code identifiers. |
...
| metadata | 메타데이터 | No | Dataset/profile descriptions. |
| method | 메소드 | No | DB Java/method execution. |
| module | 모듈 | No | IntelliJ module/project generator UI. |
...
| prompt | 프롬프트 | No | Assistant chat/user input prompts. |
| provider | 제공자 | No | AI provider. |
| pseudo column | 의사 열 | No | JDBC metadata / database column setting. |
| reference | 참조 | No | Reference identifiers. |
| region | 지역 | No | OCI region. |
| resource monitor | 리소스 모니터 | No | Connection/session monitoring UI. |
| result set | 결과 집합 | No | SQL result sets. |
| role | 역할 | No | Database role. |
| runtime | 런타임 | No | OCI runtime type. |
| savepoint | 저장점 | No | Transaction savepoint notifications. |
| schema | 스키마 | No | Database schema. |
| script | 스크립트 | No | SQL/script execution logs. |
...
| staging table | 스테이징 테이블 | No | Vector embedding staging table. |
| synonym | 동의어 | No | Database synonym object. |
| table | 테이블 | No | Database table. |
| tablespace | 테이블스페이스 | No | Oracle tablespace quota. |
| tenancy | 테넌시 | No | OCI tenancy. |
| timestamp | 타임스탬프 | No | Event notification columns. |
| TNS / tnsnames.ora | TNS / tnsnames.ora | Yes | Oracle network configuration terms. |
| token | 토큰 | No | Auth token. |
| tool | 도구 | No | Assistant/MCP tools. |
| transaction commit | 커밋 | No | Database transaction command. |
