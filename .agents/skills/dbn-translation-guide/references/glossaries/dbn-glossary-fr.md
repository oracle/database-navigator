...
Canonical terminology decisions for French DBN translation work. Keep this file compact and promote only stable, reusable terms here.

| Source term | French term | Keep English | Context / notes |
| --- | --- | --- | --- |
| action | action | No | UI actions and accessibility descriptions. |
| AI | IA | No | Standard French abbreviation. |
| API key | clé API | Yes, partial | Common French technical UI term. |
...
| connectivity | connectivité | No | Connection testing and diagnostics. |
| compartment | compartiment | No | OCI compartment. |
| configuration | configuration | No | Settings/setup surfaces. |
| connection | connexion | No | Database connection setup and runtime connection. |
| constraint | contrainte | No | Database constraints. |
| credential | identifiants | No | Authentication credential objects. |
...
| dataset | jeu de données | No | Assistant/profile datasets. |
| debug session | session de débogage | No | Debugger progress and log output. |
...
| grant | accorder des privilèges | No | Database privileges. |
| gutter | gouttière | No | IntelliJ editor gutter. |
| identifier | identifiant | No | SQL/code identifiers. |
| keyword | mot-clé | No | SQL/code keyword highlighting. |
| language model | modèle de langage | No | LLM references in prose. |
| LLM | LLM | Yes | Standard abbreviation. |
| materialized view | vue matérialisée | No | Database object type. |
| Maven | Maven | Yes | Build/runtime tool name. |
| MCP | MCP | Yes | Protocol/server name. |
| metadata | métadonnées | No | Dataset/profile descriptions. |
| method | méthode | No | DB Java/method execution. |
| module | module | No | IntelliJ module/project generator UI. |
...
| provider | fournisseur | No | AI provider. |
| pseudo column | pseudo-colonne | No | JDBC metadata / database column setting. |
| reference | référence | No | Reference identifiers. |
| region | région | No | OCI region. |
| resource monitor | moniteur de ressources | No | Connection/session monitoring UI. |
| result set | jeu de résultats | No | SQL result sets. |
| role | rôle | No | Database role. |
| runtime | environnement d’exécution | No | OCI runtime type. |
| savepoint | point de sauvegarde | No | Transaction savepoint notifications. |
| schema | schéma | No | Database schema. |
...
| timestamp | horodatage | No | Event notification columns. |
| TNS / tnsnames.ora | TNS / tnsnames.ora | Yes | Oracle network configuration terms. |
| token | jeton | No | Auth token. |
| tool | outil | No | Assistant/MCP tools. |
| transaction commit | validation | No | Database transaction command. |
