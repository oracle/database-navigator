# DBN Japanese Glossary

Canonical terminology decisions for Japanese DBN translation work. Keep this file compact and promote only stable, reusable terms here.

| Source term | Japanese term | Keep English | Context / notes |
| --- | --- | --- | --- |
| action | アクション | No | UI actions and accessibility descriptions. |
| AI | AI | Yes | Standard technical abbreviation. |
| API key | APIキー | Yes, partial | Common Japanese technical UI term. |
| argument | 引数 | No | Command/function arguments. |
...
| connectivity | 接続性 | No | Connection testing and diagnostics. |
| compartment | コンパートメント | No | OCI compartment. |
| configuration | 構成 | No | Settings/setup surfaces. |
| connection | 接続 | No | Database connection setup and runtime connection. |
| constraint | 制約 | No | Database constraints. |
| credential | 資格証明 | No | Authentication credential objects. |
...
| dataset | データセット | No | Assistant/profile datasets. |
| debug session | デバッグ・セッション | No | Debugger progress and log output. |
...
| execution privilege | 実行権限 | No | Database grants. |
...
| gutter | ガター | No | IntelliJ editor gutter. |
| identifier | 識別子 | No | SQL/code identifiers. |
...
| metadata | メタデータ | No | Dataset/profile descriptions. |
| method | メソッド | No | DB Java/method execution. |
| module | モジュール | No | IntelliJ module/project generator UI. |
...
| prompt | プロンプト | No | Assistant chat/user input prompts. |
| provider | プロバイダ | No | AI provider. |
| pseudo column | 疑似列 | No | JDBC metadata / database column setting. |
| reference | 参照 | No | Reference identifiers. |
| region | リージョン | No | OCI region. |
| resource monitor | リソース・モニター | No | Connection/session monitoring UI. |
| result set | 結果セット | No | SQL result sets. |
| role | ロール | No | Database role. |
| runtime | ランタイム | No | OCI runtime type. |
| savepoint | セーブポイント | No | Transaction savepoint notifications. |
| schema | スキーマ | No | Database schema. |
| script | スクリプト | No | SQL/script execution logs. |
...
| staging table | ステージング表 | No | Vector embedding staging table. |
| synonym | シノニム | No | Database synonym object. |
| table | 表 | No | Database table. |
| tablespace | 表領域 | No | Oracle tablespace quota. |
| tenancy | テナンシ | No | OCI tenancy. |
| timestamp | タイムスタンプ | No | Event notification columns. |
| TNS / tnsnames.ora | TNS / tnsnames.ora | Yes | Oracle network configuration terms. |
| token | トークン | No | Auth token. |
| tool | ツール | No | Assistant/MCP tools. |
| transaction commit | コミット | No | Database transaction command. |
