# DBN Spanish Glossary

Canonical terminology decisions for Spanish DBN translation work. Keep this file compact and promote only stable, reusable terms here.

| Source term | Spanish term | Keep English | Context / notes |
| --- | --- | --- | --- |
| action | acción | No | UI actions and accessibility descriptions. |
| AI | IA | No | Standard Spanish abbreviation. |
| API key | clave API | Yes, partial | Common Spanish technical UI term. |
| argument | argumento | No | Command/function arguments. |
...
| connectivity | conectividad | No | Connection testing and diagnostics. |
| compartment | compartimento | No | OCI compartment. |
| configuration | configuración | No | Settings/setup surfaces. |
| connection | conexión | No | Database connection setup and runtime connection. |
| constraint | restricción | No | Database constraints. |
| credential | credencial | No | Authentication credential objects. |
...
| dataset | conjunto de datos | No | Assistant/profile datasets. |
| debug session | sesión de depuración | No | Debugger progress and log output. |
...
| grant | conceder privilegios | No | Database privileges. |
| gutter | canal del editor | No | IntelliJ editor gutter. |
| identifier | identificador | No | SQL/code identifiers. |
| keyword | palabra clave | No | SQL/code keyword highlighting. |
| language model | modelo de lenguaje | No | LLM references in prose. |
| LLM | LLM | Yes | Standard abbreviation. |
| materialized view | vista materializada | No | Database object type. |
| Maven | Maven | Yes | Build/runtime tool name. |
| MCP | MCP | Yes | Protocol/server name. |
| metadata | metadatos | No | Dataset/profile descriptions. |
| method | método | No | DB Java/method execution. |
| module | módulo | No | IntelliJ module/project generator UI. |
...
| provider | proveedor | No | AI provider. |
| pseudo column | seudocolumna | No | JDBC metadata / database column setting. |
| reference | referencia | No | Reference identifiers. |
| region | región | No | OCI region. |
| resource monitor | monitor de recursos | No | Connection/session monitoring UI. |
| result set | conjunto de resultados | No | SQL result sets. |
| role | rol | No | Database role. |
| runtime | tiempo de ejecución | No | OCI runtime type. |
| savepoint | punto de guardado | No | Transaction savepoint notifications. |
| schema | esquema | No | Database schema. |
| script | script | Yes | SQL/script execution logs. |
...
| staging table | tabla de staging | Yes, partial | Vector embedding staging table. |
| synonym | sinónimo | No | Database synonym object. |
| table | tabla | No | Database table. |
| tablespace | tablespace | Yes | Oracle tablespace quota. |
| tenancy | tenancy | Yes | OCI tenancy term in existing bundle. |
| timestamp | marca de tiempo | No | Event notification columns. |
| TNS / tnsnames.ora | TNS / tnsnames.ora | Yes | Oracle network configuration terms. |
| token | token | Yes | Auth token. |
| tool | herramienta | No | Assistant/MCP tools. |
| transaction commit | confirmación | No | Database transaction command. |
